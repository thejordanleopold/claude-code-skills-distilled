...
- Scanning skill files on disk for embedded payloads or backdoors → use `skill-scanner`
- Auditing MCP config files or testing live tool descriptions for poisoning → use `mcp-auditor`
- Running adversarial probe suites against a system prompt → use `prompt-injection-tester`
